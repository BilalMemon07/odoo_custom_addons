                            ...
                            if uid.user_id == each.user_id:
                                ...
                                if get_user_id:
                                    ...
                                    if not duplicate_atten_ids:
                                        ...
                                        atten_date = fields.Datetime.from_string(
                                            atten_time
                                        ).date()

                                        # Find any existing hr.attendance for this employee on that date
                                        existing_att = hr_attendance.search(
                                            [
                                                ...
                                                (
                                                    "check_in",
                                                    ">=",
                                                    f"{atten_date} 00:00:00",
                                                ),
                                                (
                                                    "check_in",
                                                    "<=",
                                                    f"{atten_date} 23:59:59",
                                                ),
                                            ],
                                            order="check_in asc",
                                            limit=1,
                                        ...
                                        if each.punch == 0:  # Check-in
                                            # Check if there’s an open attendance (previous day without checkout)
                                            ...
                                            if open_att:
                                                # Mark the previous day's attendance as 'absent' (custom logic)
                                                _logger.warning(
                                                    f"{get_user_id.name} forgot to checkout on {open_att.check_in}. Marking as absent."
                                                )
                                                ...
                                                # Delete the old open attendance so the new one starts fresh
                                                open_att.unlink()

                                                # Do NOT create a check-in for the current punch
                                                continue

                                            # Otherwise, create new check-in normally
                                            if not existing_att:
                                        ...
                                        elif each.punch == 1:  # last OUT of the day
                                            if existing_att:
                                                # Always update with the latest OUT time
                                                last_out = existing_att.check_out
                                                atten_dt = fields.Datetime.from_string(
                                                    atten_time
                                                )
                                                if not last_out or atten_dt > last_out:
                                                    existing_att.write(
